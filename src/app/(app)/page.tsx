...
export default async function Dashboard() {
  ...
  const stats = [
    { label: "Saldo do mês", value: formatBRL(data.finance.balance), href: "/financas", icon: Wallet, tone: data.finance.balance < 0 ? "text-red-600" : "text-foreground" },
    { label: "Eventos hoje", value: String(data.eventsTodayCount), href: "/calendario", icon: Calendar, tone: "text-foreground" },
    { label: "Tarefas pendentes", value: String(data.pendingCount), href: "/tarefas", icon: ListChecks, tone: "text-foreground" },
    { label: "Faturas abertas", value: formatBRL(data.finance.invoices), href: "/financas", icon: CreditCard, tone: "text-amber-500" },
  ];
  ...
  return (
    <div className="max-w-5xl space-y-8">
      ...
      <div>
        ...
        <h1 className="text-4xl font-extrabold leading-none tracking-tighter text-foreground" style={{ fontFamily: "var(--font-display)" }}>
          Seu Painel
        </h1>
      </div>
          ...
          {data.week.map((d) => (
            <Link
              ...
              className={`flex flex-col items-center gap-2 rounded-xl px-1 py-3 text-center transition-colors ${
                d.isToday ? "bg-primary/8 hover:bg-primary/12" : "hover:bg-accent"
              }`}
            >
              <span className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                {weekdayOf(d.date)}
              </span>
              <span className={`flex h-7 w-7 items-center justify-center rounded-full text-sm font-semibold ${d.isToday ? "bg-primary text-primary-foreground" : "text-foreground"}`}>
                {d.day}
              </span>
              <span className={`h-1.5 w-1.5 rounded-full ${d.hasEvents ? "bg-primary" : "bg-transparent"}`} />
            </Link>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {stats.map((s) => {
          const Icon = s.icon;
          return (
            <Link key={s.label} href={s.href} className="group flex flex-col justify-between gap-6 rounded-2xl border border-border bg-card p-5 transition-colors hover:border-foreground/20">
              <div className="flex items-center justify-between">
                <Icon className="h-4 w-4 text-muted-foreground/60" strokeWidth={1.5} />
                <ArrowRight className="h-3.5 w-3.5 -translate-x-1 text-muted-foreground/0 transition-all group-hover:translate-x-0 group-hover:text-muted-foreground/40" />
              </div>
              <div>
                <p className={`text-2xl font-bold leading-none tracking-tight ${s.tone}`} style={{ fontFamily: "var(--font-display)" }}>
                  {s.value}
                </p>
      ...
      <div className="grid grid-cols-1 gap-5 lg:grid-cols-5">
        ...
        {/* finanças mini */}
        <div className="flex flex-col overflow-hidden rounded-2xl border border-border bg-card lg:col-span-2">
          ...
          <div className="flex flex-1 flex-col justify-center gap-4 p-6">
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5 text-muted-foreground"><TrendingUp className="h-3.5 w-3.5 text-green-600" /> Receitas</span>
                <span className="font-medium">{formatBRL(data.finance.income)}</span>
              </div>
              <div className="h-1.5 w-full rounded-full bg-accent">
                <div className="h-1.5 rounded-full bg-green-600" style={{ width: `${incPct}%` }} />
              </div>
            </div>
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                <span className="flex items-center gap-1.5 text-muted-foreground"><TrendingDown className="h-3.5 w-3.5 text-red-600" /> Despesas</span>
                <span className="font-medium">{formatBRL(data.finance.expense)}</span>
              </div>
              <div className="h-1.5 w-full rounded-full bg-accent">
                <div className="h-1.5 rounded-full bg-red-600" style={{ width: `${expPct}%` }} />
              </div>
            </div>
            <div className="mt-2 flex items-center justify-between border-t border-border pt-3">
              <span className="text-xs text-muted-foreground">Saldo do mês</span>
              <span className={`text-sm font-bold ${data.finance.balance < 0 ? "text-red-600" : "text-green-600"}`}>
                {formatBRL(data.finance.balance)}
              </span>
            </div>
          </div>
        </div>
            ...
            data.topTasks.map((t) => {
              ...
              return (
                <div key={t.id} className="flex items-center gap-4 px-6 py-3.5 transition-colors hover:bg-accent/50">
                  ...
                  {t.due_on && (
                    <span className={`shrink-0 text-xs ${overdue ? "font-medium text-red-600" : "text-muted-foreground"}`}>
